...
import visdom
from util.mnist_cached import MNISTCached as MNIST
from util.mnist_cached import setup_data_loaders
from util.vae_plots import mnist_test_tsne, plot_llk, plot_vae_samples
...
class VAE(nn.Module):
    ...
    def __init__(self, z_dim=20, hidden_dim=10, use_cuda=False):
        super().__init__()
...
def main(args):
    ...
    train_loader, test_loader = setup_data_loaders(
        MNIST, use_cuda=args.cuda, batch_size= 20
    )
    ...
    for epoch in range(args.num_epochs):
        ...
        if total_epoch_loss_train < best_loss:
        ...
        if epoch == args.tsne_iter:
            mnist_test_tsne(vae=vae, test_loader=test_loader, method = args.method)
            plot_llk(np.array(train_elbo), np.array(test_elbo), args.method)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-lr", "--lr_rate", default=1.0e-2, type=float, help="learning rate"
    )
    parser.add_argument(
