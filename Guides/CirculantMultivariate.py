...
class CirculantMultivariateNorm(AutoContinuous):
    ...
    def to_posdef(self, matrix):
        ...
        if matrix[0][self.latent_dim-1] > matrix[0][0]:
            matrix = matrix.fill_diagonal_((matrix[0][self.latent_dim-1] + self.residual).item() + 1) # add 0.001 for levy
        else:
            matrix = matrix.fill_diagonal_((matrix[0][0] + self.residual).item() + 1)  # add 0.001 for levy
